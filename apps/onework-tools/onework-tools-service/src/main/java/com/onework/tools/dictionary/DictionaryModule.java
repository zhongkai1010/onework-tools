 ...
 */
@Component
public class DictionaryModule implements BaseModule {
    ...
    public ModuleInfo getModuleInfo() {
        return new ModuleInfo(MODULE_CODE, "数据字典模块");
    }

    @Override
    public Map<String, String> getExceptionEnum() {

        DictionaryException[] databaseExceptions = DictionaryException.values();
        Map<String, String> messageMap = new HashMap<>(databaseExceptions.length);

        for (DictionaryException databaseException : databaseExceptions) {
