 ...
 */
@Component
public class AuthorizeModule implements BaseModule {

    /**
     * 模块异常编号
     */
    public final static String MODULE_CODE = "B001";

    @Override
    public ModuleInfo getModuleInfo() {
        return new ModuleInfo(MODULE_CODE, "权限模块");
    }

    @Override
    public Map<String, String> getExceptionEnum() {

        AuthorizeException[] databaseExceptions = AuthorizeException.values();
        Map<String, String> messageMap = new HashMap<>(databaseExceptions.length);

        for (AuthorizeException databaseException : databaseExceptions) {
