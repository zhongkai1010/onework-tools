 ...
 */
@Component
public class IdentityModule implements BaseModule {

    /**
     * 模块异常编号
     */
    public final static String MODULE_CODE = "B007";

    @Override
    public ModuleInfo getModuleInfo() {
        return new ModuleInfo(MODULE_CODE, "身份验证模块");
    }

    @Override
    public Map<String, String> getExceptionEnum() {

        IdentityException[] databaseExceptions = IdentityException.values();
        Map<String, String> messageMap = new HashMap<>(databaseExceptions.length);

        for (IdentityException databaseException : databaseExceptions) {
