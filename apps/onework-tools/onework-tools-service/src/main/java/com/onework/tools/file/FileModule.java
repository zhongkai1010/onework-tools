...
public class FileModule implements BaseModule {
     ...
     * 模块异常编号
     */
    public final static String MODULE_CODE = "B004";

    @Override
    public ModuleInfo getModuleInfo() {
        return new ModuleInfo(MODULE_CODE, "文件模块");
    }

    @Override
    public Map<String, String> getExceptionEnum() {

        FileException[] databaseExceptions = FileException.values();
        Map<String, String> messageMap = new HashMap<>(databaseExceptions.length);

        for (FileException databaseException : databaseExceptions) {
