 ...
 */
@Component
public class DomainApplicationModule implements Module {
    ...
    public Map<String, String> getErrorMessageMaps() {
        return new Hashtable<String, String>() {
            private static final long serialVersionUID = -4641807679878011904L;

            {

            put(DomainApplicationModule.DB_TYPE_IS_NULL, "数据库类型不能为空");
            put(DomainApplicationModule.CONNECTION_NAME_IS_NULL, "数据库连接名称不能为空");
            put(DomainApplicationModule.DB_HOST_IS_NULL, "数据库连接地址不能为空");
            put(DomainApplicationModule.DB_USER_IS_NULL, "数据库连接用户不能为空");
            put(DomainApplicationModule.DB_PASSWORD_IS_NULL, "数据库连接密码不能为空");
            put(DomainApplicationModule.DB_CONNECTION_ERROR, "数据库连接失败");
            put(DomainApplicationModule.DB_SCHEMA_SERVER_ERROR, "获取数据库结构服务异常");

            put(DomainApplicationModule.SYSC_CONNECTION_ERROR, "同步数据数库，数据库连接异常，连接名称：%s");
            put(DomainApplicationModule.SYSC_CONNECTION_DATABASE_ERROR, "同步数据库，数据库异常，数据库名称：%s");
            put(DomainApplicationModule.SYSC_DATABASE_CONNECTION_ERROR, "同步数据库,数据连接异常，数据库名称：%s");
            put(DomainApplicationModule.SYSC_TABLE_ERROR, "同步数据库%表异常");
        }};
    }
}
