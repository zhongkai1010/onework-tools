/*
 * @Author: 钟凯
 * @Date: 2021-03-24 09:26:45
 * @LastEditTime: 2021-03-26 09:38:53
 * @LastEditors: 钟凯
 * @Description:
 * @FilePath: \onework_manage_web\src\pages\Tool\Comparison\index.tsx
 * 可以输入预定的版权声明、个性签名、空行等
 */
import React, { useEffect, useState } from 'react';
import { PageContainer } from '@ant-design/pro-layout';
import { Row, Col, Tree, Form } from 'antd';
import { comparisonGetList, addComparison, updateComparison, deleteComparison } from '../services';
...
export default () => {
  const comparisonGetListOperate = useRequest(comparisonGetList, {
    throwOnError: true,
  });
  ...
  const [selectComparison, setSelectComparison] = useState<API.Tool.Comparison>();
  const [comparisonData, setComparisonData] = useState<API.Tool.Comparison[]>([]);
  const [form] = Form.useForm();
  useEffect(() => {
    setComparisonData(comparisonGetListOperate.data || []);
    return () => {
      setComparisonData([]);
    };
  }, [comparisonGetListOperate.data]);
      ...
      <Row gutter={18}>
        <Col span={4}>
          <ProCard
            ...
            key="treeCard"
            title="对照表"
            loading={comparisonGetListOperate.loading}
            extra={[
              <IconButton
                key="add"
                title="新增"
              ...
              <IconButton
                ...
                title="刷新"
                onClick={async () => {
                  return await comparisonGetListOperate.run();
                }}
              />,
            ]}
          >
            <Tree
              ...
              onSelect={(_selectedKeys, { node }) => {
                const temp = (node as unknown) as { source: API.Tool.Comparison };
                setSelectComparison(temp.source);
              }}
              treeData={comparisonData.map((t: API.Tool.Comparison) => {
                return {
                  key: t.uid,
                  title: t.name,
                  source: t,
