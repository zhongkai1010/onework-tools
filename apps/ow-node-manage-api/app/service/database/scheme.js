...
class SchemeService extends Service {
  constructor(ctx) {
    super(ctx);
    this.ConnectionModel = ctx.model.Database.Connection;
    this.TableModel = ctx.model.Database.Table;
    this.ColumnModel = ctx.model.Database.Column;
    this.DatabaseModel = ctx.model.Database.Database;
  }
   ...
   * @return {*} 数据库列表
   */
  async getConnectionDataBases(connection) {
    // 初始化对象
    let datas = [];
    try {
      const sequelize = this.createSequelize(connection);
      const sql = AppCode.sql[connection.dbType].database;
      const dbDatabases = await sequelize.query(sql, { type: QueryTypes.SELECT });
      ...
      if (dbDatabases.length === 0) {
        ...
        for (let i = 0; i < dbDatabases.length; i += 1) {
          const dbDatabase = dbDatabases[i];
   ...
   * @return {*} 数据库表数据
   */
  async getDatabaseTables(connection, dbName) {
  ...
  async asyncDataBase(connection, dbName) {
    ...
    const dbTables = await sequelize.query(tabSql, { type: QueryTypes.SELECT });
    ...
    for (let i = 0; i < tables.length; i += 1) {
      ...
      const dbColumns = await sequelize.query(columnSql, { type: QueryTypes.SELECT });
